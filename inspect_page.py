...
def save_inspection_files(archive, target_path):
    ...
    if main_content:
        ...
        print("[ВНИМАНИЕ] Не удалось найти стандартный блок контента (wikitext, guide-contents).")
        ...
        print(f"[ИНФО] Результат 'тупого' get_text() сохранен в: '{output_txt_get_text_file}' для анализа.")
...
def find_and_inspect(archive_path, keyword):
    ...
    for i, entry_data in enumerate(found_entries):
        print(f"  [{i+1}] Path: {entry_data['path']:<70} | Title: {entry_data['title']}")
