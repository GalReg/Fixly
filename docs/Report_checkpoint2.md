...
Бот на базе библиотеки aiogram обеспечивает пользовательский интерфейс. Он обрабатывает текстовые сообщения, управляет потоком диалога и отображает результаты пользователю в удобном формате.
