...
graph TD
    subgraph "Пользовательский интерфейс"
        A[Клиент в Telegram] -- Фото + Описание --> B(Telegram Bot API)
    end

    subgraph "Backend-система Fixly"
        B -- HTTPS Request --> C{Backend-сервис (Python)}
        C -- Запрос на анализ --> D[AI Orchestrator]
        D -- Анализ и классификация --> E{LLM Agent}
        E -- Нужны ли уточнения? --> |Да| F[Генерация вопроса]
        F -- Текст вопроса --> C
        C -- Ответ клиенту --> B
        B -- Уточняющий вопрос --> A

        E -- Нужны ли уточнения? --> |Нет, данных достаточно| G[Запрос к базе знаний]
        G -- Контекст --> H[(Vector DB / RAG)]
        H -- Релевантные документы --> E
        
        E -- Финальный промпт с контекстом --> I[Мультимодальный LLM]
        I -- Структурированный ответ (JSON) --> D
        D -- Формирование отчета --> C
    end

    subgraph "Конечный результат"
        C -- Отправка отчета --> J[Мастер]
    end
