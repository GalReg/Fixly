...
3.  **Приложение "Fixly" (Docker-контейнер):**
    ...
    *   **In-Memory Conversation Manager:** Управляет состоянием диалога для каждого пользователя (`user_memory`). Хранит историю в оперативной памяти.
...
*   **Среда исполнения:** Docker-контейнер на базе образа Python.
