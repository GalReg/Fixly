...
def main():
    ...
    logging.info(
        f"Размер одного чекпоинта: {checkpoint_size:,} чанков. Всего чекпоинтов: {num_checkpoints}."
    )
    ...
    for i in range(0, total_chunks, checkpoint_size):
        ...
        if os.path.exists(batch_path):
            logging.info(
                f"Чекпоинт {batch_num}/{num_checkpoints} уже существует. Пропускаем."
            )
            continue
        ...
        all_embeddings = []
        progress_bar = tqdm(
            texts_to_embed, total=len(texts_to_embed), desc=f"Чекпоинт {batch_num}"
        )
        ...
        for text in progress_bar:
            try:
                ...
                all_embeddings.append(
                    [0.0] * 256
                )  # 256 - размерность эмбеддингов YandexGPT

        # Убедимся, что мы получили эмбеддинги
        if not all_embeddings or len(all_embeddings) != len(texts_to_embed):
            logging.error(
                f"В батче {batch_num} не удалось создать эмбеддинги. Пропускаем сохранение."
            )
            continue
        ...
        db = FAISS.from_embeddings(
            text_embedding_pairs, embeddings_model, metadatas=metadatas
        )
    ...
    logging.info(
        f"Теперь запустите скрипт 'merge_vector_db.py', чтобы объединить их в финальную базу в '{FINAL_DB_PATH}'."
    )
