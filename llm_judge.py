...
def print_metrics(metrics: Dict[str, Dict[str, float]], prefix: str = ""):
    ...
    for criterion, values in metrics.items():
        print(
            f"{prefix}{criterion:35s} | Mean: {values['mean']:5.2f} | Median: {values['median']:5.2f} | Count: {values['count']:4d}"
        )
...
async def process_dialogues():
    ...
    for task in async_tqdm.as_completed(
        tasks, total=len(tasks), desc="Оценка диалогов"
    ):
        await task
    ...
    with open(REPORT_FILE, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"Успешно обработано: {sum(1 for r in results if r.get('evaluation'))}\n"
        )
