...
def inspect_random(archive_path, start_index, num_samples):
    ...
    if start_index >= total_entries:
        print(
            f"Ошибка: Начальный индекс ({start_index}) больше или равен общему числу записей ({total_entries})."
        )
        return
    ...
    if len(sampling_range) < num_samples:
        print(
            f"Внимание: В диапазоне с {start_index} по {start_index + 50000} всего {len(sampling_range)} записей."
        )
