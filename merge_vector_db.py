...
if not YANDEX_API_KEY or not YANDEX_FOLDER_ID:
    raise ValueError(
        "YANDEX_API_KEY и YANDEX_FOLDER_ID должны быть установлены в .env файле"
    )
...
def merge_faiss_indexes():
    ...
    if not os.path.exists(TMP_DB_PATH) or not os.listdir(TMP_DB_PATH):
        logging.error(
            f"Папка с временными индексами '{TMP_DB_PATH}' пуста или не существует."
        )
        logging.error(
            "Сначала запустите 'create_vector_db.py' для создания чекпоинтов."
        )
        return
    ...
    batch_folders = sorted(
        [
            os.path.join(TMP_DB_PATH, d)
            for d in os.listdir(TMP_DB_PATH)
    ...
    try:
        ...
        main_db = FAISS.load_local(
            batch_folders[0], embeddings_model, allow_dangerous_deserialization=True
        )
    ...
    for folder in tqdm(batch_folders[1:], desc="Объединение индексов"):
        try:
            db_to_merge = FAISS.load_local(
                folder, embeddings_model, allow_dangerous_deserialization=True
            )
            main_db.merge_from(db_to_merge)
        except Exception as e:
            logging.warning(
                f"Не удалось загрузить или объединить индекс из '{folder}'. Ошибка: {e}. Пропускаем."
            )
    ...
    logging.info(
        f"Финальная векторная база данных успешно сохранена в '{FINAL_DB_PATH}'."
    )
    logging.info(
        "Теперь вы можете использовать ее в своем боте. Папку 'vectorstore/tmp' можно удалить для экономии места."
    )
