...
OUTPUT_DIR = (
    "/Users/rrgalieva/Documents/fixly_bot/ifixit_data"  # Новая папка для чистых данных
)
    ...
    if steps_container:
        ...
        for step in steps:
            ...
            if step_content_div:
                ...
                step_content_text = step_content_div.get_text(
                    separator="\n", strip=True
                )
                step_text += f"\n{step_content_text}"
...
def main():
    ...
    for i in range(total_entries):
        if i > 0 and i % 10000 == 0:
            print(
                f"Обработано {i} из {total_entries} записей. Найдено и сохранено: {processed_count}"
            )
